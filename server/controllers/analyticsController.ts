...
export async function getDashboardStats(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const averageRoadmapProgress = totalRoadmaps > 0 
      ? Math.round(roadmapsData.reduce((acc, r) => acc + r.progressPercentage, 0) / totalRoadmaps)
      : 0;

    // 4. Mood distribution (focused, happy, tired, productive, stressed)
    const moodCounts: { [key: string]: number } = { focused: 0, happy: 0, tired: 0, productive: 0, stressed: 0 };
    ...
    return res.status(500).json({ success: false, message: 'Failed to compute analytics.', error: error.message });
  }
}
