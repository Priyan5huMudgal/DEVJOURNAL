...
async function createRoadmap(req, res) {
  ...
  if (!title) {
    return res
      .status(400)
      .json({ success: false, message: "Please provide a roadmap title." });
  }
  try {
    ...
    const estComp = estimatedCompletion
...
async function updateRoadmap(req, res) {
  ...
  try {
    ...
    if (estimatedCompletion)
      roadmap.estimatedCompletion = new Date(estimatedCompletion);
    if (topics) {
      roadmap.topics = topics.map((t, index) => ({
        name: t.name,
        status: t.status || "todo",
        order: t.order || index + 1,
      }));
...
async function updateTopicStatus(req, res) {
  ...
  if (!status) {
    return res
      .status(400)
      .json({ success: false, message: "Please provide the topic status." });
  }
  try {
    const roadmap = await Roadmap.findOne({ _id: id, userId });
    if (!roadmap)
    ...
    if (isNaN(index) || index < 0 || index >= roadmap.topics.length) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid topic index." });
    }
